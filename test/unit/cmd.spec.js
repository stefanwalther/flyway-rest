...
describe( 'cli:buildCommand', ()=> {
  ...
  it( 'cli:buildCommand:command has to one of ["clean", "info", "validate", "baseline", "repair", "migrate"]', () => {
    expect( cmd.buildCommand( { url: 'foo' }, 'clean' ) ).to.be.equal( 'flyway --url foo clean' );
    expect( cmd.buildCommand( { url: 'foo' }, 'info' ) ).to.be.equal( 'flyway --url foo info' );
    expect( cmd.buildCommand( { url: 'foo' }, 'validate' ) ).to.be.equal( 'flyway --url foo validate' );
